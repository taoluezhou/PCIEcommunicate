...
void MainWindow::on_write_clicked()
{
    ...
    int d;
    d = (ui->Numwrite->toPlainText()).toUInt();

    int p;
    p = (ui->Addwrite->toPlainText()).toUInt();
...
void MainWindow::on_DMAread_clicked()
{
 ...
 //   const int nTestSize = 16*101;
    const int nTestSize = 256*100;
    const PCIE_LOCAL_ADDRESS LocalAddr = 0;        
    BYTE *pRead = new BYTE[nTestSize];
    int p=ui->AddDMAread->toPlainText().toInt();
    int count=ui->CountTextEdit->toPlainText().toInt();
